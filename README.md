...
[![Offline](https://img.shields.io/badge/Plugins-Offline_Packed-green)](#-特性一览)
...
> 💡 **最简单的方式：直接双击 `install.bat`**，它会自动完成全部安装步骤——包括自动下载安装 **DSH Desktop v2.0.0**（若本机未装）、复制全部配置与离线插件、安装依赖，全程不需要打开终端、不需要敲任何命令。安装完成后窗口会自动提示结果。
...
>
> 🌐 **关于联网需求**：所有 17 个自研/第三方插件均已随仓库离线分发，**不访问 GitHub 下载插件**；仅 DSH Desktop 本体（首次安装时）与 npm registry 标准依赖需要联网。
