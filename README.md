...
- **客户端**：若本机未安装 DSH Desktop，脚本会**通过国内高速镜像自动下载安装**；也可以从官网一键下载：[https://www.dshdesktop.cn/](https://www.dshdesktop.cn/)
...
> 💡 **最省心的方式：直接双击 `install.bat`**。它会自动为你完成全套流程：
> 1. 检测客户端：若没有则走**国内高速镜像**秒级自动下载并静默安装；
> 2. 注入 17 个离线插件、CAD 审查套件、火线战队工作台、提示词与预设；
> 3. 完成本地依赖构建，双击即全套就绪！

| 方式 | 操作 | 适合人群 |
| :--- | :--- | :--- |
| **🖱️ 双击（强烈推荐）** | 在文件资源管理器中**双击 `install.bat`** | 队友最省心，全程全自动 |
